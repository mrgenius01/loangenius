        ...
        <View style={styles.statusContainer}>
          <View style={styles.statusHeader}>
            <Text style={[styles.statusIcon, { color: getStatusColor() }]}>
              {getStatusIcon()}
            </Text>            <Text style={[styles.statusText, { color: getStatusColor() }]}>
              {(status && status.status) ? status.status.toUpperCase() : 'CHECKING...'}
            </Text>
          </View>          <View style={styles.statusDetails}>
            <View style={styles.statusRow}>
              <Text style={styles.statusLabel}>Reference:</Text>
              <Text style={styles.statusValue}>{transaction.reference || 'N/A'}</Text>
            </View>
            ...
            <View style={styles.statusRow}>
              ...
              <Text style={styles.statusValue}>${transaction.amount || '0.00'}</Text>
            </View>
            ...
            {status && (              <>
                <View style={styles.statusRow}>
                  ...
                  <Text style={[styles.statusValue, { color: getStatusColor(), fontWeight: 'bold' }]}>
                    {(status && status.status) ? status.status.toUpperCase() : 'UNKNOWN'}
                  </Text>
                </View>

                <View style={styles.statusRow}>
                  <Text style={styles.statusLabel}>Paid:</Text>
                  <Text style={[styles.statusValue, { color: (status && status.paid) ? '#10b981' : '#ef4444', fontWeight: 'bold' }]}>
                    {(status && status.paid) ? 'YES' : 'NO'}
                  </Text>
                </View>
              </>
            )}
    ...
    <CustomModal visible={visible} onClose={onClose} title="OMari Payment" type="info">
      {transaction && (
        <View style={styles.otpContainer}>
          <View style={styles.otpHeader}>
          ...
          </View>          <View style={styles.otpDetails}>
            <View style={styles.statusRow}>
              <Text style={styles.statusLabel}>Reference:</Text>
              <Text style={styles.statusValue}>{transaction.reference || 'N/A'}</Text>
            </View>
            ...
            <View style={styles.statusRow}>
              ...
              <Text style={styles.statusValue}>{transaction.phone_number || 'N/A'}</Text>
            </View>
          </View>
...
const DashboardScreen = ({ onLogout }) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      }); if (response.data.status === 'success') {        const transactionData = {
          ...response.data,
          amount: amount,
          method: selectedMethod,
