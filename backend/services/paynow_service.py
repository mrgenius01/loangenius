...
class PaynowService:
    ...
    def create_payment(self, reference, amount, phone_number, method):
        ...
        """
        # Use a real email for authemail (required by Paynow)
        authemail = os.getenv('PAYNOW_AUTH_EMAIL', 'user@example.com')
        # Format amount as string with two decimal places
        formatted_amount = "{:.2f}".format(float(amount))
        payment = self.paynow.create_payment(reference, authemail)
        payment.add('Loan Repayment', formatted_amount)
        # Set only _info, not info, to avoid breaking the method
        try:
            payment._info = "Loan Repayment"
        except Exception:
            pass
        
        # DEBUG: Log the actual payload sent to Paynow
        try:
            paynow_payload = {
                "resulturl": self.paynow.result_url,
                "returnurl": self.paynow.return_url,
                "reference": payment.reference,
                "amount": formatted_amount,
                "id": self.paynow.integration_id,
                "additionalinfo": payment._info if hasattr(payment, '_info') else (payment.info if isinstance(payment.info, str) else payment.info()),
                "authemail": authemail,
                "phone": phone_number,
                "method": method,
                "status": "Message"
            }
            for key, value in paynow_payload.items():
                if key in ('authemail', 'returnurl', 'resulturl'):
                    continue
                paynow_payload[key] = requests.utils.quote(str(value))
            from services.hash_service import generate_paynow_hash
            debug_sdk_hash = generate_paynow_hash(paynow_payload, self.integration_key)
            print(f"[DEBUG] Actual Paynow SDK payload: {paynow_payload}")
            print(f"[DEBUG] Hash generated by SDK logic: {debug_sdk_hash}")
        except Exception as e:
            print(f"[DEBUG] Could not log SDK payload/hash: {e}")
        
        # DEBUG: Show payment data and hash before sending to Paynow
        payment_data = {
            'reference': reference,
            'email': authemail,  # Use the same email as in the actual payment
            'amount': formatted_amount,
            'integration_key': self.integration_key
        }
        debug_hash = self.hash_service.generate_hash(payment_data)
        print(f"[DEBUG] Payment data for hash: {payment_data}")
        print(f"[DEBUG] Generated hash for payment: {debug_hash}")
...
class PaynowResponse:
    ...
    def __str__(self):
        # Return a readable string of the main response data
        return f"PaynowResponse(success={self.success}, method={self.method}, data={self.data}, remoteotpurl={self.remoteotpurl}, otpreference={self.otpreference}, redirect_url={self.redirect_url})"
    
    def __repr__(self):
        # Return a dictionary representation for debugging
        return f"PaynowResponse({self.response.__dict__}, method={self.method})"
